...
#=====================布林带(Bollinger Band)====================
import talib
def getBBands(df,period,nbdevup,nbdevdn):
    close=df['cl']
...
hyperPars=[10,2,2]
Boll=getBBands(pr,10,2,2)
stg= pr.join(Boll, how='outer')

def buysigTiming(hyperPars=[]):
    sig=pd.DataFrame()
    buySig=[]
    sellSig=[]
    for i in range(hyperPars[0]):    #range(hyperPars[0])      数据处理（前几列数据为空）
        buySig.append(-1)
        sellSig.append(-1)

    for i in range(hyperPars[0],len(stg)):    #i=hyperPars[0]      #timeperiod前hyperPars[0]数据为空，舍去
