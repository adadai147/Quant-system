...
#=======================基于DC与SMA的择时策略======================
#1.m周期移动平均线与唐奇安中心线在n周期移动平均线的中心线以上交叉并收盘时，买入股票
#2.m周期移动平均线与唐奇安中心线在n周期移动平均线的中心线以下交叉并收盘时，卖出股票
#超参：m,n,period
#=====================ATR(Average True Range)====================


buyTimingDC_SMA <- function(x,hyperPars=c(m,n,p)){
  py$period=hyperPars[3]
  py_run_file("C:/量化投资/2022.10-11/Donchian Channel（DC）/SMA/buyTimingDC_SMA_py.py")
  stg <- x[,{
    MA_m <- SMA(x$op,hyperPars[1])        #计算出m日的移动平均值
    MA_n <- SMA(x$op,hyperPars[2])        #计算出n日的移动平均值
    mband<-py$mband
    buySig<-ifelse((pr$cl>MA_m)&
                     (((shift(MA_m,1)<mband)&(MA_m>mband))|
                      ((shift(MA_m,1)>mband)&(MA_m<mband))),1,0)
    sellSig<-ifelse((pr$cl<MA_m)&
                     (((shift(MA_m,1)<mband)&(MA_m>mband))|
                        ((shift(MA_m,1)>mband)&(MA_m<mband))),1,0)
...
#策略函数测试
#stgy <- buyTimingDC_SMA(x=pr,hyperPars=c(50,10,10))
#stgy 
...
parSpace <- expand_grid(m,n,p) %>% data.table #生成参数搜索空间，一行对应一套参数
...
Search <-function(parSpace){
  Search <- parSpace[,{
    cat('m=',m,' n=',n,' p=',p,'\n')
    ppv <- evaluateppv(buyTimingDC_SMA(x=pr,hyperPars=c(m=m,n=n,p=p)))
    ppv
  },by=.(m,n,p)] #每个参数组合都检验，能暴力选出ppv最好的参数组合
  setnames(Search,'V1','ppv')
  Search  #查看参数组合以及其所对应的ppv值
  Best <- Search[which.max(ppv)]
  return(Best)    #查看最优参数组合s
}
Search(parSpace)
...
X <- hyperParSpace[,{
  cat('m=',m,' n=',n,' p=',p,'\n')
  stg <-buyTimingDC_SMA(x=pr,
                     hyperPars=c(m=m,n=n,p=p)) #首先在全数据上生成买入卖出信号
...
},by=.(m,n,p)] # 针对每一个超参组合，计算全部训练集J和对应的全部验证集Jc上的ppv结果
...
Ld <- X[,{
  ...
  lambda <- log(w/(1-w))
  list(lambda=lambda)         
},by=.(m,n,p)]# 针对每一个训练集，计算样本外测试集的ppv相对排名，并计算lambda
