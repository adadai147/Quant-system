...
buyTimingDC_M <- function(x,hyperPars=c(n)){
  py$period=hyperPars[1]
  py_run_file("C:/量化投资/2022.10-11/Donchian Channel（DC）/hband_lband/buyTimingDC_HL_py.py")
  stg <- x[,{
    mband<-py$mband
    cl<-pr$cl
    buySig<-ifelse((shift(cl,1)<mband)&(cl>mband),1,0)
    sellSig<-ifelse((shift(cl,1)>mband)&(cl<mband),1,0)
...
#策略函数测试
#stgy <- buyTimingDC_M(x=pr,hyperPars=c(20))
#stgy                        
