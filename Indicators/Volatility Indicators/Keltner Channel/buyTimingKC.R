...
#=======================基于KC的择时策略======================
#1.当市场价格突破肯特纳通道的上轨，表示市场价格强势，上涨趋势将延续一段时间，买入股票
#2.当市场价格突破肯特纳通道的下轨，表示市场价格将继续下跌一段时间，卖出股票
#超参：m-window,n-window_atr,x-multiplier
#=====================KC(Keltner Channel)====================


buyTimingKC <- function(x,hyperPars=c(m,n,x)){
  py$window=hyperPars[1]
  py$window_atr=hyperPars[2]
  py$multiplier=hyperPars[3]
  py_run_file("C:/量化投资/2022.10-11/Keltner Channel（KC）/buyTimingKC_py.py")
  stg <- x[,{
    hband<-py$hband
    lband<-py$lband
    buySig<-ifelse((hband>pr$cl) & (shift(hband,1)<shift(pr$cl,1)),1,0)
    sellSig<-ifelse((lband<pr$cl) & (shift(lband,1)>shift(pr$cl,1)),1,0)
...
#策略函数测试
#stgy <- buyTimingKC(x=pr,hyperPars=c(20,10,2))
#stgy 
...
parSpace <- expand_grid(m,n,x) %>% data.table #生成参数搜索空间，一行对应一套参数
...
X <- hyperParSpace[,{
  cat('m=',m,' n=',n,' x=',x,'\n')
  stg <-buyTimingKC(x=pr,
                        hyperPars=c(m=m,n=n,x=x)) #首先在全数据上生成买入卖出信号
...
},by=.(m,n,x)] # 针对每一个超参组合，计算全部训练集J和对应的全部验证集Jc上的ppv结果
...
Ld <- X[,{
  ...
  lambda <- log(w/(1-w))
  list(lambda=lambda)         
},by=.(m,n,x)]# 针对每一个训练集，计算样本外测试集的ppv相对排名，并计算lambda
