...
#==============基于Rate of Change (ROC)的择时策略函数==================
#超参： n天前的收盘价，持有时间holdTime
buyTimingROC <- function(x,hyperPars=c(n,holdTime)){
  py$n=hyperPars[1]
  py_run_file("D:/Quant/Rcode/Indicators/Rate of Change(ROC)/ROC_py.py")
  stg <- x[,{
    PPO <- py$roc
    buySig <- ifelse(PPO>0 & PPO*shift(PPO,1)<0,1,0)
    # 如果价格上穿KAMA，就给出买入信号
    sellSig <- shift(buySig,hyperPars[2])
    list(date,op,cl,buySig,sellSig)
  }] 
  return(stg)
}
#stg <- buyTimingPPO(pr,c(12,2))
...
parSpace <- expand_grid(n,holdTime) %>% data.table
...
X <- hyperParSpace[,{
  cat(' n=',n,'holdTime=',holdTime,'\n')
  stg <-buyTimingROC(x=pr,
                     hyperPars=c(n,holdTime)) #首先在全数据上生成买入卖出信号
...
},by=.(n,holdTime)] # 针对每一个超参组合，计算全部训练集J和对应的全部验证集Jc上的ppv结果
