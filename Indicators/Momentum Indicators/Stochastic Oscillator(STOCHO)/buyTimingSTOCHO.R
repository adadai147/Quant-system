...
#==============基于Stochastic Oscillator的择时策略函数==================
#超参： n1:backward period n2:smooth period
buyTimingSTOCHO <- function(x,hyperPars=c(n1,n2)){
  py$n1=hyperPars[1];py$n2=hyperPars[2]
  py_run_file("D:/Quant/Rcode/Indicators/Stochastic Oscillator(STOCHO)/STOCHO_py.py")
  stg <- x[,{
    PPO <- py$sto
    buySig <- ifelse(PPO<20,1,0)

    sellSig <- ifelse(PPO>80,1,0)
    list(date,op,cl,buySig,sellSig)
  }] 
  return(stg)
}
#stg <- buyTimingSTOCHO(pr,c(14,3))
