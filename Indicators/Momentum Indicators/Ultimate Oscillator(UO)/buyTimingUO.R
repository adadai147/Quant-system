...
#==============基于Ultimate Oscillator (UO)的择时策略函数==================
#超参： n1:backward period n2:smooth period
buyTimingUO <- function(x,hyperPars=c(short,medium,long)){
  py$short=hyperPars[1];py$medium=hyperPars[2];py$long=hyperPars[3]
  py_run_file("D:/Quant/Rcode/Indicators/Ultimate Oscillator(UO)/UO_py.py")
  stg <- x[,{
    uo <- py$uo
    buySig <- ifelse(uo>50 & shift(uo,1)<50,1,0)
    
    sellSig <- ifelse(uo<50 & shift(uo,1)>50,1,0)
    list(date,op,cl,buySig,sellSig)
  }] 
  return(stg)
}
#stg <- buyTimingUO(pr,c(7,14,28))
...
short <- seq(5,10,by=1)
medium <- seq(12,18,by=1)
long <- seq(20,26,by=1)

parSpace <- expand_grid(short,medium,long) %>% data.table
...
X <- hyperParSpace[,{
  cat(' short=',short,'medium=',medium,'long=',long,'\n')
  stg <-buyTimingUO(x=pr,
                     hyperPars=c(short,medium,long)) #首先在全数据上生成买入卖出信号
...
},by=.(short,medium,long)] # 针对每一个超参组合，计算全部训练集J和对应的全部验证集Jc上的ppv结果
