...
#==============基于Relative Strength Index (RSI)的择时策略函数==================
#超参： n:period
buyTimingSTOCHRSI <- function(x,hyperPars=c(n,para1,para2)){
  py$n=hyperPars[1];py$para1=hyperPars[2];py$para2=hyperPars[3]
  py_run_file("D:/Quant/Rcode/Indicators/Stochastic Relative Strength Index (STOCHRSI)/STOCHRSI_py.py")
  stg <- x[,{
    PPO <- py$rsi
    buySig <- ifelse(PPO<0.2,1,0)
    # 如果价格上穿KAMA，就给出买入信号
    sellSig <- ifelse(PPO>0.8,1,0)
    list(date,op,cl,buySig,sellSig)
  }] 
  return(stg)
}
stg <- buyTimingSTOCHRSI(pr,c(14,3,3))
...
para1 <- seq(3,5,by=1)
para2 <- seq(3,5,by=1)
parSpace <- expand_grid(n,para1,para2) %>% data.table
...
X <- hyperParSpace[,{
  cat(' n=',n,'\n')
  stg <-buyTimingSTOCHRSI(x=pr,
                     hyperPars=c(n,para1,para2)) #首先在全数据上生成买入卖出信号
...
},by=.(n,para1,para2)] # 针对每一个超参组合，计算全部训练集J和对应的全部验证集Jc上的ppv结果
