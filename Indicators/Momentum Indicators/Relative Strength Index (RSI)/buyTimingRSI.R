...
#超参： n:period
buyTimingRSI <- function(x,hyperPars=c(n)){
  py$n=hyperPars[1]
  py_run_file("D:/Quant/Rcode/Indicators/Relative Strength Index (RSI)/RSI_py.py")
  stg <- x[,{
    PPO <- py$rsi
    buySig <- ifelse(PPO<30,1,0)
    # 如果价格上穿KAMA，就给出买入信号
    sellSig <- ifelse(PPO>70,1,0)
    list(date,op,cl,buySig,sellSig)
  }] 
  return(stg)
}
stg <- buyTimingRSI(pr,c(14))
