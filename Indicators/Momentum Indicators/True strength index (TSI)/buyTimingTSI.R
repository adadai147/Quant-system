...
#==============基于True strength index (TSI)的择时策略函数==================
#超参： slow,fast:high/low period  n: days of SMA(tsi)
buyTimingTSI <- function(x,hyperPars=c(slow,fast,n)){
  py$slow=hyperPars[1];py$fast=hyperPars[2]
  py_run_file("D:/Quant/Rcode/Indicators/True strength index (TSI)/TSI_py.py")
  stg <- x[,{
    tsi <- py$tsi
    tsi_s <- SMA(tsi,hyperPars[3])
    buySig <- ifelse(tsi>0 & tsi-tsi_s>0,1,0)
    
    sellSig <- ifelse(tsi<0 & tsi-tsi_s<0,1,0)
    list(date,op,cl,buySig,sellSig)
  }] 
  return(stg)
}
#stg <- buyTimingTSI(pr,c(25,13,9))
...
fast <- seq(5,15,by=1)
n <- seq(4,14,by=1)
parSpace <- expand_grid(slow,fast,n) %>% data.table
...
X <- hyperParSpace[,{
  cat(' slow=',slow,'fast=',fast,'\n')
  stg <-buyTimingTSI(x=pr,
                        hyperPars=c(slow,fast,n)) #首先在全数据上生成买入卖出信号
...
},by=.(slow,fast,n)] # 针对每一个超参组合，计算全部训练集J和对应的全部验证集Jc上的ppv结果
