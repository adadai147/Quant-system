...
#==============基于Force Index (FI)的择时策略函数==================
#超参： lag:days of SMA 
buyTimingFI <- function(x,hyperPars=c(n1,n2)){
  py$n1=hyperPars[1];py$n2=hyperPars[2]
  py_run_file("D:/Quant/Rcode/Indicators/Force Index (FI&CCI)/FI_py.py")
  stg <- x[,{
    fi <- py$fi
    cci <- py$cci
    buySig <- ifelse(fi>0 & cci<(-100),1,0)
    sellSig <- ifelse(fi<0 & cci>100,1,0)
    list(date,op,cl,buySig,sellSig)
  }] 
  return(stg)
}
#stg <- buyTimingFI(pr,c(14))
