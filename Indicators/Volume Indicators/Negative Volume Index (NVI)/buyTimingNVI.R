...
#==============基于Negative Volume Index (NVI)的择时策略函数==================
#超参： lag:days of SMA 
buyTimingNVI <- function(x,hyperPars=c(n)){
  py$n=hyperPars[1]
  py_run_file("D:/Quant/Rcode/Indicators/Money Flow Index (MFI)/MFI_py.py")
  stg <- x[,{
    nvi <- py$nvi
    buySig <- ifelse(SMA(pr$vol,20)>40000 & mfi>SMA(mfi,hyperPars[1]),1,0)
    #超卖
    sellSig <- ifelse(SMA(pr$vol,20)>40000 & mfi<SMA(mfi,hyperPars[1]),1,0)
    #超买
    list(date,op,cl,buySig,sellSig)
  }] 
  return(stg)
}
#stg <- buyTimingNVI(pr,c(14))
