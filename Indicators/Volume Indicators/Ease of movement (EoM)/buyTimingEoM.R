...
#==============基于Ease of movement (EoM, EMV)的择时策略函数==================
#超参： lag:days of SMA 
buyTimingEoM <- function(x,hyperPars=c(n)){
  py$n=hyperPars[1]
  py_run_file("D:/Quant/Rcode/Indicators/Ease of movement (EoM)/EoM_py.py")
  stg <- x[,{
    eom <- py$eom
    buySig <- ifelse(eom>0&shift(eom,1)<0&eom>SMA(eom,hyperPars[1]),1,0)
    sellSig <- ifelse(eom<0&shift(eom,1)>0&eom<SMA(eom,hyperPars[1]),1,0)
    list(date,op,cl,buySig,sellSig)
  }] 
  return(stg)
}
#stg <- buyTimingEoM(pr,c(14))
