...
#==============基于Money Flow Index (MFI)的择时策略函数==================
#超参： lag:days of SMA 
buyTimingFI <- function(x,hyperPars=c(n)){
  py$n=hyperPars[1]
  py_run_file("D:/Quant/Rcode/Indicators/Money Flow Index (MFI)/MFI_py.py")
  stg <- x[,{
    mfi <- py$mfi
    buySig <- ifelse(SMA(pr$vol,20)>40000 & SMA(pr$cl,60)>20 & mfi<10,1,0)
    #超卖
    sellSig <- ifelse(SMA(pr$vol,20)>100000 & SMA(pr$cl,60)>20 & mfi>90,1,0)
    #超买
    list(date,op,cl,buySig,sellSig)
  }] 
  return(stg)
}
#stg <- buyTimingMFI(pr,c(14))
