...
buyTiming <- function(x,hyperPars=c(n,k_buy,k_sell,d)){#n为设定区间，k_buy,k_sell为买卖判断斜率）
  py$n=hyperPars[1];k_buy=hyperPars[2];k_sell=hyperPars[3];d=hyperPars[4]
  py_run_file("D:/codes/EMA/EMA_py.py")
  stg <- x[,{
    ema <- py$ema
    buySig <- ifelse((shift(ema,d)-ema)/d > k_buy,1,0)
    sellSig <- ifelse((shift(ema,d)-ema)/d < k_sell,1,0)
    # 
...
#====================================================================================================
n <- seq(10,90,by=10)
k_buy <- seq(0,0.3,by=0.03)
k_sell <- seq(-0.3,0,by=0.03)
d <- seq(1,5,by=1)
parSpace <- expand_grid(n,k_buy,k_sell,d) %>% data.table
...
X <- hyperParSpace[,{
  cat('n=',n,' k_buy=',k_buy,' k_sell=',k_sell,' d=',d,'\n')
  stg <-buyTiming(x=pr,
                  hyperPars=c(n=n,k_buy=k_buy,k_sell=k_sell,d=d)) #首先在全数据上生成买入卖出信号
...
},by=.(n,k_buy,k_sell,d)] # 针对每一个超参组合，计算全部训练集J和对应的全部验证集Jc上的ppv结果
