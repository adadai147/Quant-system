...
Search <-function(parSpace){
  Search <- parSpace[,{
    ...
    ppv <- evaluateSR(buysigTiming(x=pr,hyperPars=c(M1=M1,N1=N1,M2=M2,N2=N2,n=n)))
    ppv
  },by=.(M1,N1,M2,N2,n)] #每个参数组合都检验，能暴力选出ppv最好的参数组合
  setnames(Search,'V1','SR')
  Search  #查看参数组合以及其所对应的ppv值
  Best <- Search[which.max(SR)]
...
Ld <- X[,{
  ...
  lambda <- log(w/(1-w))
  list(lambda=lambda)         
},by=.(M1,M2,N1,N2,n)]# 针对每一个训练集，计算样本外测试集的ppv相对排名，并计算lambda
